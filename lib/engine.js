 ...
 * @param {{ idScope: string, sasToken: string, registrationHost: string }} context 
...
module.exports = async function (context, device, measurements, properties, timestamp) {
    ...
    try {
        ...
        if(measurements) {
            console.log('[%s] Sending telemetry for device ', TRANSPORT_TYPE, device.deviceId);
            await client.sendEvent(message);
            console.log('[%s] Telemetry sent for ', TRANSPORT_TYPE,device.deviceId);
        }        
        
        if (properties) {
            console.log('[%s] Get twin', TRANSPORT_TYPE,device.deviceId);
            var twin = await client.getTwin();
            console.log('[%s] Obtained twin for device ', TRANSPORT_TYPE,device.deviceId);
            await twin.properties.reported.update(properties);
            console.log('[%s] Updated twin for device ', TRANSPORT_TYPE,device.deviceId);
        }

        console.log('[%s] Closing client for ', TRANSPORT_TYPE,device.deviceId);
        await client.close();
        console.log('[%s] Client closed for ', TRANSPORT_TYPE,device.deviceId);
...
async function getDeviceKey(context, deviceId) {
    ...
    const key = crypto.createHmac('SHA256', Buffer.from(context.sasToken), 'base64')
        .update(deviceId)
        .digest()
        .toString('base64');
